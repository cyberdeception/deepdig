...
if __name__ == '__main__':
    ...
    train_label = np.asarray([0 if x == 1 or x ==6  else 1 for x in train_label])
    ...
    test_label =np.asarray( [0 if x == 1 or x ==6  else 1 for x in test_label])

    print(len(test_label))
    rtio = 0.80

    train_feature, test_feature, train_label, test_label = train_test_split(train_feature, train_label, train_size=rtio)
