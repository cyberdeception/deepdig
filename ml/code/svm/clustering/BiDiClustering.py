...
class BiDiClustering:
    ...
    def traceToInstance( trace ):
        ...
        for packet in trace.getPackets():
            ...
            if packet.getDirection()!=directionCursor:
                ...
                if config.GLOVE_OPTIONS['burstSize'] == 1:
                    ...
                    dataKey = 'S'+str(directionCursor)+'-'+str( BiDiClustering.roundArbitrary(dataCursor, config.bucket_Size) )
                ...
                if config.GLOVE_OPTIONS['burstNumber'] == 1:
                    numberKey = 'N'+str(directionCursor)+'-'+str( BiDiClustering.roundNumberMarker(numberCursor) )
        ...
        if dataCursor>0:
            ...
            if config.GLOVE_OPTIONS['burstSize'] == 1:
                ...
                key = 'S'+str(directionCursor)+'-'+str( BiDiClustering.roundArbitrary(dataCursor, config.bucket_Size) )
        ...
        htmlMarker = BiDiClustering.roundArbitrary( htmlMarker, 600 )
        ...
        bandwidthUp   = BiDiClustering.roundArbitrary( trace.getBandwidth( Packet.UP ),   10000)
        bandwidthDown = BiDiClustering.roundArbitrary( trace.getBandwidth( Packet.DOWN ), 10000)
        ...
        for packet in trace.getPackets():
            ...
            if packet.getDirection()!=directionCursor:
                ...
                if config.GLOVE_OPTIONS['burstTime'] == 1:
                    timeKey = 'T'+str(directionCursor)+'-'+str( BiDiClustering.roundArbitrary(timeCursor, timeBase) )
                ...
                if secondBurstAndUp:
                    ...
                    if config.GLOVE_OPTIONS['biBurstSize'] == 1:
                        pairBurstDataKey = 'biSize-'+str(prevDirectionCursor)+'-'+str(directionCursor)+'-'+ \
                                         str( BiDiClustering.roundArbitrary(prevDataCursor, sizeBase) )+'-'+ \
                                         str( BiDiClustering.roundArbitrary(dataCursor, sizeBase) )
                    ...
                    if config.GLOVE_OPTIONS['biBurstTime'] == 1:
                        biBurstTimeKey = 'biTime-'+str(prevDirectionCursor)+'-'+str(directionCursor)+'-'+ \
                                         str( BiDiClustering.roundArbitrary(prevTimeCursor, timeBase) )+'-'+ \
                                         str( BiDiClustering.roundArbitrary(timeCursor, timeBase) )
    ...
    def classify( runID, trainingSet, testingSet ):
        ...
        if (config.CLUSTERING_METHOD==1):
            clusteringAPI.calcKmeans([trainingFile,testingFile],config.NUM_CLUSTERS,"Description goes here!")
        elif (config.CLUSTERING_METHOD==2):
            #[trainingFile,testingFile] = Utils.calcPCA2([trainingFile,testingFile])
            [trainingFile,testingFile] = Utils.calcPCA_ow([trainingFile,testingFile])
            clusteringAPI.calcKmeans([trainingFile,testingFile],"Description goes here!")
        elif (config.CLUSTERING_METHOD==3):
            #[trainingFile,testingFile] = Utils.calcPCA2([trainingFile,testingFile])
            [trainingFile,testingFile] = Utils.calcPCA_ow([trainingFile,testingFile])
            clusteringAPI.calcKmeansCvxHullDelaunay([trainingFile,testingFile],config.NUM_CLUSTERS,"Description goes here!")
        elif (config.CLUSTERING_METHOD==4):
            #[trainingFile,testingFile] = Utils.calcPCA2([trainingFile,testingFile])
            [trainingFile,testingFile] = Utils.calcPCA_ow([trainingFile,testingFile])
            clusteringAPI.calcKmeansCvxHullDelaunay_Mixed([trainingFile,testingFile],config.NUM_CLUSTERS,"Description goes here!")
        elif (config.CLUSTERING_METHOD==5):
            #[trainingFile,testingFile] = Utils.calcPCA2([trainingFile,testingFile])
            [trainingFile,testingFile] = Utils.calcPCA_ow([trainingFile,testingFile])
            clusteringAPI.calcKmeansCvxHullDelaunay_Mixed_KNN([trainingFile,testingFile],config.NUM_CLUSTERS,"Description goes here!", threshold=3)
