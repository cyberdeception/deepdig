...
class KNN_LCS:
    ...
    def calcHP_KNN_LCS(self):
        ...
        self.__Yprediction = neigh.predict(np.arange(len(self.__Xtest)).reshape(-1, 1)) # an index into a separate data structure (Xtest).
                                                                                        # neigh.predict compares each testing instance against all training instance using LCS_dist metric
